...
st.set_page_config(page_title="Sanal Drone Emlak Video Oluşturucu", 
                   layout="wide", 
...
from config.config import initialize_session_state, VOICE_OPTIONS, DEFAULT_VOICE, VIDEO_DIR
...
class EmlakVideoApp:
    ...
    def setup_sidebar(self):
        ...
        with st.sidebar:
            ...
            st.session_state['current_view'] = st.radio(
                "Arayüz Görünümü",
                options=['tabs', 'wizard'],
                format_func=lambda x: "Sekmeli Görünüm" if x == 'tabs' else "Adım Adım Rehber",
                index=0 if st.session_state.get('current_view') == 'tabs' else 1
            )
            
            # Video ayarları
            st.session_state['fps'] = st.slider("Kare Hızı (FPS)", 15, 60, 30, 
    ...
    def setup_address_input(self):
        ...
        address = st.text_input("Emlak adresi:", placeholder="Örnek: Atatürk Mah. Cumhuriyet Cad. No:123, İstanbul")
    ...
    def check_background_tasks(self):
        ...
        if 'video_task_id' in st.session_state:
            ...
            if task_status:
                ...
                if task_status['status'] in ['starting', 'running', 'preparing', 
                                            'processing_images', 'generating_video']:
                ...
                # Hata durumunda göster
                    ...
                    st.error(f"Video oluşturulurken bir hata meydana geldi: {task_status['error']}")
                    ...
                    if st.button("🔄 Yeniden Dene", help="Video oluşturmayı tekrar deneyin"):
                        del st.session_state['video_task_id']
                        st.experimental_rerun()
    ...
    def save_project(self):
        ...
        with st.form(key="save_project_form"):
            ...
            if submit_button:
                ...
                # Projeyi kaydet
                success = self.state_manager.save_project(project_name, project_data, overwrite)
                
                if success:
                    st.success(f"Proje '{project_name}' başarıyla kaydedildi!")
                else:
                    st.error(f"Proje '{project_name}' kaydedilemedi. Aynı isimli bir proje zaten mevcut.")
    ...
    def show_tabbed_interface(self):
        ...
        tabs = st.tabs(["1. Emlak Bilgileri", "2. Sesli Anlatım", "3. Görüntüler", "4. Video Oluştur"])
...
class PropertyController:
    ...
    def show_property_form(self):
        ...
        with col1:
            property_type = st.selectbox("Emlak Tipi:", ["Daire", "Villa", "Müstakil Ev", "Arsa", "Ticari", "Diğer"])
        ...
        with col2:
            ...
            year_built = st.number_input("Yapım Yılı:", min_value=1900, max_value=2025, value=2010)
        
        special_features = st.text_area("Özel Özellikler:", 
        ...
        if st.button("Emlak Metni Oluştur", type="primary"):
            self._generate_description(property_type, rooms, bathrooms, area, price, year_built, special_features, nearby_places)
    ...
    def _get_nearby_info(self):
        ...
        if include_nearby:
            nearby_radius = st.slider("Araştırma Yarıçapı (metre)", 500, 2000, 1000, 100)
            with st.spinner("Yakın çevre analizi yapılıyor..."):
                nearby_places = get_nearby_places(
                    ...
                    types=["school,hospital,shopping_mall,park,restaurant,subway_station,bus_station"]
                )
                
                if nearby_places:
                    with st.expander("Yakındaki Önemli Noktalar"):
                        for place in sorted(nearby_places, key=lambda x: x['distance']):
                            st.write(f"🏢 **{place['name']}** ({place['type'].replace('_', ' ')}) - {place['distance']}m")
        
        return nearby_places
    
    def _generate_description(self, property_type, rooms, bathrooms, area, price, year_built, special_features, nearby_places):
        ...
        with st.spinner("Metin oluşturuluyor..."):
            ...
            generated_text = generate_property_description(
                property_data, 
                nearby_places if nearby_places else None
            )
...
class ImageController:
    ...
    def _show_map_images(self):
        ...
        if 'maps_images' in st.session_state and len(st.session_state['maps_images']) > 0:
    ...
    def _fetch_map_images(self, zoom_level, map_type):
        ...
        with st.spinner("Görüntüler alınıyor..."):
            ...
            main_img = fetch_satellite_image(lat=lat, lng=lng, zoom=zoom_level, maptype=map_type)
            if main_img:
                # Renk iyileştirmeyi uygula
                if st.session_state['enhance_colors']:
                    main_img = enhance_image(main_img, boost_factor=st.session_state['color_boost'])
                ...
                for i, zoom in enumerate(zoom_levels):
                    img = fetch_satellite_image(lat=lat, lng=lng, zoom=zoom, maptype=map_type)
                    if img:
                        # Renk iyileştirmeyi uygula
                        if st.session_state['enhance_colors']:
                            img = enhance_image(img, boost_factor=st.session_state['color_boost'])
    ...
    def _show_border_drawing(self):
        ...
        st.info("Uydu görüntüsü üzerinde emlak sınırını belirlemek için aşağıdaki ayarları kullanın.")
        
        # Sınır çizilecek görüntüyü seç
        image_options = [f"Görüntü {i+1}" for i in range(len(st.session_state['maps_images']))]
        selected_img_idx = st.selectbox("Sınır çizilecek görüntüyü seçin:", 
        ...
        with col_color:
            ...
            color_key = st.selectbox("Sınır Rengi:", 
        ...
        with col_ratio:
            border_ratio = st.slider("Sınır Konumu:", 0.05, 0.45, 0.2, 
        ...
        if st.button("Sınırı Çiz", type="primary"):
            with st.spinner("Sınır çiziliyor..."):
                bordered_image = draw_property_border(selected_image, color_key, border_width, border_ratio)
                ...
                st.image(bordered_image, caption="Sınırlı Emlak Görüntüsü", use_container_width=True)
                
                # Videoya dahil etme seçeneği
                include_in_video = st.checkbox("Bu görüntüyü videoya dahil et", value=True)
                if include_in_video:
                    ...
                    new_maps_images.insert(0, bordered_image)  # Sınırlı görüntüyü ilk sıraya ekle
    ...
    def _show_custom_images(self):
        ...
        if uploaded_files:
            ...
            for idx, img in enumerate(user_images):
                st.image(img, caption=f"Özel Görsel {idx + 1}", use_container_width=True)
...
class VideoController:
    ...
    def show_video_generation(self):
        ...
        tabs = st.tabs(["Temel Ayarlar", "Arkaplan Müziği", "Metin/Logo Ekle"])
        ...
        with tabs[0]:
            ...
            resolution_option = st.selectbox(
                ...
                index=0 if st.session_state.get('video_quality', 'normal') == 'normal' else 1
            )
            st.session_state['video_quality'] = "normal" if "720p" in resolution_option else "high"
    ...
    def _show_music_options(self):
        ...
        if selected_music == "custom":
            uploaded_music = st.file_uploader("Kendi müziğinizi yükleyin:", type=["mp3", "wav", "ogg"])
            if uploaded_music:
                # Geçici dosya oluştur
                temp_dir = tempfile.mkdtemp()
                music_path = os.path.join(temp_dir, f"custom_music.{uploaded_music.name.split('.')[-1]}")
            ...
            st.session_state["background_music_type"] = selected_music
        else:
            st.session_state.pop("background_music_path", None)
            st.session_state.pop("background_music_type", None)
            
        # Müzik ses seviyesi seçeneği
        if selected_music != "no_music":
            music_volume = st.slider("Müzik Ses Seviyesi", 0.0, 1.0, 0.2, step=0.05, 
                                   help="Düşük değerler anlatımı öne çıkarır, yüksek değerler müziği daha belirgin yapar")
            st.session_state["background_music_volume"] = music_volume
    ...
    def _show_overlay_options(self):
        ...
        use_text_overlay = st.checkbox("Videoya Metin Ekle", value=st.session_state.get("use_text_overlay", False))
        ...
        use_logo_overlay = st.checkbox("Videoya Logo Ekle", value=st.session_state.get("use_logo_overlay", False))
        ...
        if use_logo_overlay:
            ...
            if uploaded_logo:
                ...
                with col2:
                    ...
                    logo_opacity = st.slider("Logo Şeffaflığı:", 0.1, 1.0, 0.8, step=0.1)
                    st.session_state["overlay_logo_opacity"] = logo_opacity
    
    def _generate_video(self):
        """Video oluştur ve göster"""
        try:
            # Seçilen tüm görüntüleri birleştir
            all_images = []
            ...
            # Görüntülere metin ve logo ekle
            if st.session_state.get("use_text_overlay") or st.session_state.get("use_logo_overlay"):
                with st.spinner("Görüntülere metin/logo ekleniyor..."):
                    # Metin ekleme
                    text_options = None
                    if st.session_state.get("use_text_overlay") and "overlay_text" in st.session_state:
                        text = st.session_state.get("overlay_text", "")
                        text_position = st.session_state.get("overlay_text_position", "bottom")
                        text_color = st.session_state.get("overlay_text_color", "#FFFFFF")
                    ...
                    # Logo ekleme
                    logo_options = None
                    if st.session_state.get("use_logo_overlay") and "overlay_logo" in st.session_state:
                        logo = st.session_state["overlay_logo"]
                        logo_position = st.session_state.get("overlay_logo_position", "bottom-right")
                        logo_size = st.session_state.get("overlay_logo_size", 15)
                        logo_opacity = st.session_state.get("overlay_logo_opacity", 0.8)
                    ...
                    for img in all_images:
                        if st.session_state.get("use_text_overlay") and "overlay_text" in st.session_state and text_options:
                            img = add_text_overlay(img, st.session_state["overlay_text"], **text_options)
                        if st.session_state.get("use_logo_overlay") and "overlay_logo" in st.session_state and logo_options:
                            img = add_logo_overlay(img, st.session_state["overlay_logo"], **logo_options)
            ...
            if st.session_state.get("background_music_type") and st.session_state["background_music_type"] != "no_music":
                with st.spinner("Arkaplan müziği hazırlanıyor..."):
                    music_type = st.session_state["background_music_type"]
                    
                    if music_type == "custom" and "background_music_path" in st.session_state:
                        music_path = st.session_state["background_music_path"]
                    else:
                        music_path = download_music(music_type)
                    
                    if music_path:
                        music_volume = st.session_state.get("background_music_volume", 0.2)
                        mixed_audio_path = mix_audio(audio_path, music_path, music_volume)
                        
                        if mixed_audio_path:
                            audio_path = mixed_audio_path
                            st.success("Müzik ve sesli anlatım başarıyla karıştırıldı.")
            
            # Videoyu arka planda oluştur
            task_id = self.task_manager.start_task(
                generate_video_in_background,
                task_args=(all_images, audio_path, st.session_state['transition_type'], 
                        st.session_state['fps'], st.session_state['video_quality']),
                task_name="video_generation"
            )
            
            # Görev ID'sini sakla
            st.session_state['video_task_id'] = task_id
            st.success("Video oluşturma işlemi başlatıldı. Lütfen bekleyin...")
                
        except Exception as e:
            st.error(f"Video oluşturulurken hata oluştu: {str(e)}")
